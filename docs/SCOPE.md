...
10. **ترنسپورت اپ Android آینده**: کلاینت رسمی Convex (`dev.convex:android-convexmobile`) — `ConvexClient` با mutation/subscribe→Flow؛ بدون Retrofit/REST دستی. احراز هویت هم بدون تغییر بک‌اند: action عمومی `auth:signIn` (OTP دو مرحله‌ای + refresh با refreshToken) از خود ConvexClient فراخوانی می‌شود — پروتکل تأییدشده از سورس `@convex-dev/auth`؛ جزئیات و نمونه کد Kotlin در `docs/MOBILE_API.md` بخش ۰ و ۱.
11. **اسکلت اندروید ساخته شد** (`android/`): Gradle + Manifest + Application + SessionStore رمزنگاری‌شده + AuthRepository (OTP) + صف آفلاین راننده (DataStore، idempotencyKey تغییرناپذیر) + SyncWorker (WorkManager، backoff) + ParentRepository + FCM service + **UI کامل Compose**: DriverScreen (انتخاب سرویس، roster زنده، دکمه‌های یک‌لمسی، نشانگر Pending Sync) و ParentScreen (وضعیت فرزندان، تایم‌لاین، اطلاعات راننده/خودرو) + مسیریابی نقش‌محور در MainActivity (bootstrap:myContext). نام همه توابع بک‌اند در کد از سورس Convex تأیید شده (recordEvent/registerDevice = mutation؛ myChildren/serviceRoster/myServices/myContext = query). **کامپایل Kotlin در این محیط اجرا نشده** — باز شدن در Android Studio لازم است.

## SCALE_TARGETS.md

اعداد معماری (۵۵۶ eps میانگین خام در مقیاس ۱M دانش‌آموز) فقط Target هستند و بدون Benchmark واقعی Verified اعلام نمی‌شوند.

### نتایج Benchmark واقعی (اجراشده روی deployment زنده)

اندازه‌گیری: درج دسته‌ای رویداد در یک تراکنش واحد، زمان از سمت کلاینت (شامل network round-trip):

| تعداد رویداد | زمان کل | توان عبوری |
|---|---|---|
| ۵۰۰ | ۷۷۹ms | **۶۴۱ eps** ✅ (Target 500 Verified در این حالت) |
| ۱٬۰۰۰ | ۹۶۴ms | **۱٬۰۳۷ eps** ✅ |
| ۲٬۰۰۰ | ۱٬۵۲۳ms | **۱٬۳۱۳ eps** ✅ |
| ۵٬۰۰۰ | ۳٬۰۶۴ms | **۱٬۶۳۱ eps** ✅ |

**Caveats (مهم):** این اعداد توان عبوری «نوشتن تراکنشی دسته‌ای تک‌اتصال» را می‌سنجند، نه بار همزمان توزیع‌شده. سناریوی واقعی peak (۱M راننده همزمان) به اجرای موازی روی چند اتصال نیاز دارد که در این محیط شبیه‌سازی نشده — status آن بخش همچنان **Not measured** است. اعداد بالا نشان می‌دهند مسیر نوشتن DB در تراکنش واحد حداقل ۵۰۰–۱٬۶۰۰ eps را پوشش می‌دهد.

همه ردیف‌های بنچمارک بعد از تست حذف شدند (تأیید: count = 0).
